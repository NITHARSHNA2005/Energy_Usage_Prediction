...
@st.cache_resource
def load_artifacts():
    base_path = os.path.join(os.getcwd(), "models", "saved_cnn_bilstm_model")
...
if "predict" in st.session_state and st.session_state["predict"]:
    ...
    if st.session_state["selected_mode"] == "specific_hour":
        hour = st.session_state.get("hour")
        if hour is None:
            st.error("Please select an hour and generate prediction again.")
            st.stop()
